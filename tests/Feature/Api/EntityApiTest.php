...
test('entity can be created', function () {
    ...
    $response = $this->actingAs($user)->postJson("/api/entities/$entity_model->route", $entity->toArray());
...
test('Duplicate entity can not be created', function () {
    ...
    $response = $this->actingAs($user)->postJson("/api/entities/$entity_model->route", $entity2->toArray());
